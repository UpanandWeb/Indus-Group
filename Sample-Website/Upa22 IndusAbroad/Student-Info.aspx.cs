...
public partial class Student_Info : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            ...
            else
            {
                ...
                if (ds2.Tables[0].Rows.Count > 0)
                {
                    for (int i = 1; i <= ds2.Tables[0].Rows.Count; i++)
                    {
                        ...
                        bcountry.DataTextField = "vcountryname";
                        bcountry.DataValueField = "icountryid";
                        bcountry.DataSource = ds2;
                        bcountry.DataBind();
                        bcountry.Items.Insert(0, "Select Country");

                        ncountry.DataTextField = "vcountryname";
                        ncountry.DataValueField = "icountryid";
                        ncountry.DataSource = ds2;
                        ncountry.DataBind();
                        ncountry.Items.Insert(0, "Select Country");

                        pcountry.DataTextField = "vcountryname";
                        pcountry.DataValueField = "icountryid";
                        pcountry.DataSource = ds2;
                        pcountry.DataBind();
                        pcountry.Items.Insert(0, "Select Country");

                        ccountry.DataTextField = "vcountryname";
                        ccountry.DataValueField = "icountryid";
                        ccountry.DataSource = ds2;
                        ccountry.DataBind();
                        ccountry.Items.Insert(0, "Select Country");

                              country.DataTextField = "vcountryname";
                        country.DataValueField = "icountryid";
                ...
                SqlDataReader dr;
                string qry = "select * from IA_Studentinfo where EmailId='" + emailid + "'";
            ...
            SqlDataAdapter sdaaaaa = new SqlDataAdapter("select * from Studentdoc where emailid='" + emailid + "' order by did asc", con);
    ...
    public DataSet GetEmpDet2(string emailid)
    {
        ...
        SqlDataAdapter da = new SqlDataAdapter("select * from IA_Studentinfo where emailid='" + emailid + "'", connection);
        DataSet ds = new DataSet();
        da.Fill(ds);
        //close database connection to disconnect to sql server
        connection.Close();
    ...
    protected void btnsubmit_Click(object sender, EventArgs e)
    {
        ...
        if (filedoc.HasFile)
        {
            ...
            for (int i1 = 2; i1 < fileCollection.Count; i1++)
            {
                ...
                string path = Server.MapPath("~/admin/Student_Photos/" + strUserid + fileName);
                filedoc.SaveAs(path);
...
bacname, acnumber, bname, badd, bifsc, bswift, bacadd, adhar, pan, dnumber, dplace, dcity, strUserid, marital, acity, aplace, pplace, pcity);
    ...
    {
        con.Open();
        string s = "insert into   Studentdoc values('" + imgname23 + "','" + strUserid + "')";
...
bacname, acnumber, bname, badd, bifsc, bswift, bacadd, adhar, pan, dnumber, dplace, dcity, strUserid, marital, acity, aplace, pplace, pcity);
        ...
        if (i == 1)
        {
            string strscpt = "alert ('Your Details Saved Successfully');location.replace('Emp_Profile.aspx');";
            Page.ClientScript.RegisterStartupScript(this.GetType(), "alert", strscpt, true);
        }
    }
    ...
    protected void bstate_SelectedIndexChanged(object sender, EventArgs e)
    {
        if (bstate.SelectedItem.Text.ToString() != "Select State")
        {
            ...
            if (ds.Tables[0].Rows.Count > 0)
            {
                ...
                {
                    bcity.Items.Add("Select");
                    bcity.Items[i].Text = ds.Tables[0].Rows[i - 1]["cityname"].ToString();
    ...
    protected void state_SelectedIndexChanged(object sender, EventArgs e)
    {
        if (state.SelectedItem.Text.ToString() != "Select State")
        {
            ...
            if (ds.Tables[0].Rows.Count > 0)
            {
                ...
                {
                    city.Items.Add("Select");
                    city.Items[i].Text = ds.Tables[0].Rows[i - 1]["cityname"].ToString();
    ...
    public DataSet GetCounty()
    {
        DataSet ds = new DataSet();
        con.Open();
        SqlDataAdapter sqlda = new SqlDataAdapter("select * from ia_Countries order by vcountryname Asc", con);
        sqlda.Fill(ds);
        con.Close();
        return ds;
    }
    public DataSet GetState(string id)
    {
        ...
        con.Open();
        SqlDataAdapter sqlda = new SqlDataAdapter("select * from State_Master where intcountryid = " + id + " order by txtstate Asc", con);
        sqlda.Fill(ds);
        con.Close();
        return ds;
    }
    public DataSet GetCity(string id1)
    {
        ...
        con.Open();
        SqlDataAdapter sqlda1 = new SqlDataAdapter("select * from City_Master where sid = " + id1 + " order by cityname Asc", con);
        sqlda1.Fill(ds1);
        con.Close();
        return ds1;
    }
    ...
    protected void lnkphoto_Click(object sender, EventArgs e)
    {
        ...
        string Query = "";
        Query = "select userid,photo,photopath from IA_Studentinfo where username='" + emailid + "'";
    ...
    protected void lnkresume_Click(object sender, EventArgs e)
    {
        ...
        string Query = "";
        Query = "select userid,filename1,rpath from IA_Studentinfo where username='" + emailid + "'";
        SqlDataAdapter sdaupload = new SqlDataAdapter(Query, con);
        con.Open();
        SqlCommand cmd = new SqlCommand(Query, con);
        SqlDataReader dr = cmd.ExecuteReader();
            ...
            if (filedoc.HasFile)
            {
                ...
                if ((strextension == ".gif") || (strextension == ".jpeg") || (strextension == ".jpg") || (strextension == ".bmp") || (strextension == ".JPEG") || (strextension == ".png") || (strextension == ".PNG") || (strextension == ".JPG") || (strextension == ".GIF"))
                {
                    ...
                    if (j == 1)
                    {
                        fppath = "";
                        ddc.SelectedIndex = -1;
                        SqlDataAdapter sdaaaaa = new SqlDataAdapter("select * from Studentdoc where emailid='" + strUserid + "' order by did asc", con);
                        DataSet dssss = new DataSet();
                        sdaaaaa.Fill(dssss);
                        if (dssss.Tables[0].Rows.Count > 0)
                        {
    ...
    {
        con.Open();
        string s = "insert into   Studentdoc values('" + imgname23 + "','" + strUserid + "','" + course + "')";
        SqlCommand cmd = new SqlCommand(s, con);
        int i = cmd.ExecuteNonQuery();
        con.Close();
        return i;
    ...
    protected void lnkc_Command(object sender, CommandEventArgs e)
    {
        ...
        string Query = "";
        Query = "select * from Studentdoc where did='" + aux + "'";
        SqlDataAdapter sdaupload = new SqlDataAdapter(Query, con);
        con.Open();
        SqlCommand cmd = new SqlCommand(Query, con);
        SqlDataReader dr = cmd.ExecuteReader();
