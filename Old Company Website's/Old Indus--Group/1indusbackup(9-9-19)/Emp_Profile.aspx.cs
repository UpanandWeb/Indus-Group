...
public partial class Emp_Profile : System.Web.UI.Page
{
    ...
    ////ed5, ed5b, ed5s, ed5y, ed5p, ed6, ed6b, ed6s, ed6y, ed6p, ed7, ed7b, ed7s, ed7y, ed7p, 
    ////,imgName1, date, fppath1, imgName, fppath,
    //,bacname, acnumber, bname, badd, bifsc, bswift, bacadd, dnumber, dplace, dcity, strUserid, marital, fsn, spssn, k1sn, k2sn, k3sn);

    //        if (i == 1)
    //        {
    //            string strscpt = "alert ('Your Details Updated');location.replace('Emp_Profile.aspx');";
    ...
    //      string ed6y, string ed6p, string ed7, string ed7b, string ed7s, string ed7y, string ed7p,
    //string imgName, string fppath, string bacname, string acnumber, string bname,
    ...
    protected void lnkc_Command(object sender, CommandEventArgs e)
    {
        ...
        string Query = "";
        Query = "select * from EmployeeEducation  where sno='" + aux + "'";
        SqlDataAdapter sdaupload = new SqlDataAdapter(Query, con);
        con.Open();
        SqlCommand cmd = new SqlCommand(Query, con);
        SqlDataReader dr = cmd.ExecuteReader();
    ...
    //    TextBox Education = (TextBox)row.Cells[0].Controls[0].FindControl("txtedu");
    //    TextBox Board = ((TextBox)row.Cells[1].Controls[0].FindControl("txtboard"));
    //    TextBox major = ((TextBox)row.Cells[2].Controls[0].FindControl("txtmajor"));
    //    TextBox yearofpass = ((TextBox)row.Cells[3].Controls[0].FindControl("txtyearofpass"));
    //    TextBox percentage = ((TextBox)row.Cells[4].Controls[0].FindControl("txtpercentage"));
    //    con.Open();
    //    SqlCommand cmd = new SqlCommand("Update EmployeeEducation set Educational='" + Education.Text + "',Board/University='" + Board.Text + "',Majorin='" + major.Text + "',        YearOfPass ='" + yearofpass.Text + "',percentage='" + percentage.Text + "'", con);
    //    cmd.ExecuteNonQuery();
    //    con.Close();
    ...
    protected void btnsubmit1_Click(object sender, EventArgs e)
    {
        ...
        try
        {
            ...
            string vistype = ddvisatype.Text;
    ...
    public void Bindgrid()
    {
        ...
        {
            emailid = Session["userid"].ToString();
            SqlDataAdapter sdaaaaa = new SqlDataAdapter("select * from EmployeeEducation   where Emaiid ='" + emailid + "' order by sno asc", con);
            DataSet dssss = new DataSet();
            sdaaaaa.Fill(dssss);
            if (dssss.Tables[0].Rows.Count > 0)
            {
                GridView2.DataSource = dssss;
                GridView2.DataBind();
            }
        }
        catch (Exception ex)
    ...
    //    string intialland = ((DropDownList)row.Cells[0].Controls[0]).Text;
    //    string fromind = ((DropDownList)row.Cells[1].Controls[0]).Text;
    //    string datearrival = ((TextBox)row.Cells[2].Controls[0]).Text;
    //    string i94 = ((TextBox)row.Cells[3].Controls[0]).Text;
    //    string vistype = ((TextBox)row.Cells[4].Controls[0]).Text;

    //    string reason = ((TextBox)row.Cells[5].Controls[0]).Text;
    //    string stayfrom = ((TextBox)row.Cells[6].Controls[0]).Text;
    //    string stayto = ((TextBox)row.Cells[7].Controls[0]).Text;
    //    string visastamp = ((DropDownList)row.Cells[8].Controls[0]).Text;
    //    string passport = ((DropDownList)row.Cells[9].Controls[0]).Text;


    //    con.Open();
    //    SqlCommand cmd = new SqlCommand("Update IG_Immigrant set Initial_Landing_USA='" + intialland + "',From_Location='" + fromind + "',Date_Arrived='" + datearrival + "',        I94='" + i94 + "',Visa_Type='" + vistype + "',Reason_Travel='" + reason + "',Stay_From='" + stayfrom + "',Stay_To='" + stayto + "',Visa_Stamp_Location='" + visastamp + "',        Passport_Issued_Country='" + passport + "')", con);
    //    //cmd.ExecuteNonQuery();
    //    con.Close();
    //    GridView1.EditIndex = -1;
    //    subgrid();
    ...
    protected void OnCancel1(object sender, EventArgs e)
    {
        //if (ViewState["currtable"] != null)
        //{
        //    GridView1.EditIndex = -1;
    ...
    protected void GridView2_RowEditing1(object sender, System.Web.UI.WebControls.GridViewEditEventArgs e)
    {
        //if (ViewState["currtable"] != null)
        //{
        //    GridView1.EditIndex = e.NewEditIndex;
    ...
    protected void btnsubmit2_Click(object sender, EventArgs e)
    {
        ...
        {
            emailid = Session["userid"].ToString();
            string Education = eduList1.SelectedItem.Text;
            string Board = ed1brd.Text;
            string major = ed1sub.Text;
            string yearofpass = ed1yr.Text;
            string percentage = ed1per.Text;
            string date = DateTime.UtcNow.ToString();
            con.Open();
            string s = "insert into   EmployeeEducation  values('" + emailid + "','" + Education + "','" + Board + "','" + major + "','" + yearofpass + "','" + percentage + "','" + date + "' )";
            SqlCommand cmd = new SqlCommand(s, con);
            int i = cmd.ExecuteNonQuery();
            con.Close();
            Clearfileds();
    ...
    {
        //Educational Details Page//
        //eduList1.Items.Clear();
        //eduList1.Items.Add(new System.Web.UI.WebControls.ListItem("--Please Select--", "0"));
        //this.eduList1.Items.Insert(0, "Please select");
        eduList1.SelectedValue = string.Empty;
        //eduList1.SelectedItem.Text = "";
        ed1brd.Text = "";
        ed1sub.Text = "";
        ed1yr.Text = "";
        ed1per.Text = "";
        ...
        //Non Immigrant Travel Record//
        dduscitis.SelectedValue = string.Empty;
        ddindcitis.SelectedValue = string.Empty;
        txtdateofarrival.Text = "";
        txti94.Text = "";
        ddvisatype.SelectedValue = string.Empty;
        txtreason.Text = "";
        txtstayfrom.Text = "";
        txtstayto.Text = "";
        ddvisastamp.SelectedValue = string.Empty;
        ddindpassport.SelectedValue = string.Empty;
    ...
    protected void GridView2_RowDeleting(object sender, GridViewDeleteEventArgs e)
    {
        ...
        {
            GridViewRow row = (GridViewRow)GridView2.Rows[e.RowIndex];
            Label lbldeleteid = (Label)row.FindControl("lblID1");
            // int index = Convert.ToInt32(e.RowIndex);
            con.Open();
            SqlCommand cmd = new SqlCommand("delete FROM EmployeeEducation where sno='" + lbldeleteid.Text + "'", con);
            cmd.ExecuteNonQuery();
            con.Close();
            Bindgrid();
        }
    ...
    protected void GridView2_RowUpdating(object sender, GridViewUpdateEventArgs e)
    {
        try
        {
            ...
            TextBox percentage = ((TextBox)row.Cells[4].Controls[0].FindControl("txtpercentage"));
            con.Open();
            // SqlCommand cmd = new SqlCommand("Update EmployeeEducation set Educational='" + Education.Text + "',Board/University='" + Board.Text + "',Major in='" + major.Text + "',        Year Of Pass='" + yearofpass.Text + "',percentage='" + percentage.Text + "' where sno='" + lblID1.Text + "'", con);
            SqlCommand cmd = new SqlCommand("Update EmployeeEducation set Education='" + Education.Text + "',Board='" + Board.Text + "',major='" + major.Text + "',        yearofpass ='" + yearofpass.Text + "',percentage='" + percentage.Text + "' where sno='" + lblID1.Text + "'", con);
            // SqlCommand cmd = new SqlCommand("Update EmployeeEducation set Educational='" + Education.Text + "',Board/University='" + Board.Text + "',Majorin='" + major.Text + "',        YearOfPass ='" + yearofpass.Text + "', percentage='" + percentage.Text + "' where sno='" + lblID1.Text + "'", con);
            cmd.ExecuteNonQuery();
            con.Close();
            GridView2.EditIndex = -1;
            Bindgrid();
