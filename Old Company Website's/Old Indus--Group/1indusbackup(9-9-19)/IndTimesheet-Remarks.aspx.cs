...
public partial class IndTimesheet_Remarks : System.Web.UI.Page
{
    ...
    protected void btnsaveout_Click(object sender, EventArgs e)
    {
        if (ddtype.SelectedItem.Text != "Select")
        {
            if (ddtype.SelectedItem.Text != "Leave")
            {
                ...
                {
                    con.Open();
                    string s = "insert into IA_DailytimeSheet(empname,empid,empemail,ddate,status,country,posteddate,remark1,worktime,intime,outtime,Lunch,intime1,outtime1,mrate,rsymbol) values ('" + name + "','" + empid + "','" + email + "','" + date + "','4','" + ddcont + "','0','" + remark + "','0','00:00','00:00','00:00','00:00','00:00','" + rate + "','" + symbol + "') ";
                    SqlCommand cmd = new SqlCommand(s, con);
                    int i = cmd.ExecuteNonQuery();
                    con.Close();
                    if (i == 1)
                    {
                        string strscpt = "alert ('Leave Details Saved');location.replace('Indemp-DailyTimesheet.aspx');";
    ...
    {
        Response.Redirect("Indemp-DailyTimesheet.aspx?userid=" + Session["userid"].ToString());
    }
}